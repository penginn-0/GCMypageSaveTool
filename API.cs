...
namespace GCMypageSaveTool
{
    internal class GCAPI
    {
        ...
        public void GetPlayedMusics()
        {
            ...
            sb.Clear();
            sb.AppendLine(
                "楽曲ID\t楽曲名\t楽曲名カナ\tアーティスト\t使用スキン\t楽曲プレイ回数\t最終プレイ日時\tお気に入り\t" +
            ...
            foreach (var Music in MusicList.music_list)
            {
                ...
                sb.Append($"{MusicData.music_detail.music_id}\t{MusicData.music_detail.music_title}\t{Music.kana}\t{MusicData.music_detail.artist}\t{MusicData.music_detail.skin_name}\t{Music.play_count}\t{Music.last_play_time}\t{MusicData.music_detail.fav_flg}\t");
